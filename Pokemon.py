...
class Pokemon:
    ...
    def damage_against(self, enemy, pvp=1, weather=None, friendship=1):
        ...
        mod = lambda m: te(m) * stab(m) * wab(m) * shadow * fsb
    ...
    def cycle_dps_against(self, enemy, pvp=1, weather=None, friendship=1):
        ...
        cnt_dur_dmg = zip(self.charge_count(pvp), self.charge_duration(pvp), damage[1:])
        return [(cnt * damage[0] + dmg) / dur for cnt, dur, dmg in cnt_dur_dmg]
